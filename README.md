# Lineup Lab v0.7 — Premium UI + GitHub共有 + MP4自動圧縮
...
- 攻略ツールとして安っぽく見えない、情報階層の明確な高品質UIにする。

## 使用者

個人・友達同士でVALORANTの定点を共有する用途を想定しています。
アカウント管理や投稿審査などの大規模サービス向け機能は入れていません。

## v0.7 UIデザイン方針

v0.7では機能追加よりも見た目と操作感を優先して再設計しています。

- VALORANTのクライアントに合う暗色 + 赤アクセントを維持
- 単純な濃紺カードの並びではなく、背景・パネル・入力欄・カード・マップの階層を分離
- 極端に小さい文字を減らして可読性を改善
- ボタン、入力欄、フィルター、アクティブ状態を統一
- マップを画面中央の主役として見せる
- 定点カードと詳細パネルの情報密度を整理
- 作成画面のフォーム、画像、MP4領域の視認性を改善
- マイ定点カードとデータ管理モーダルの質感を統一
- 過剰なアニメーションや重い背景画像は追加しない
- 1280px前後でも主要機能が致命的に崩れないレスポンシブ調整
- `prefers-reduced-motion` に対応

UIは `assets/js/premium-ui.js` から追加スタイルを読み込みます。
既存のHTML構造・定点データ・主要JSロジックは変更せず、見た目を分離して保守しやすくしています。
...
そのため、IndexedDBへの保存完了後なら元のPC上のMP4ファイルを削除しても、同じブラウザでは再生できます。
...
通常の定点閲覧ではFFmpeg coreを読み込まず、**圧縮ボタンを押した時だけ**圧縮エンジンを取得します。

初回圧縮時は約31MBのFFmpeg core取得が発生します。
動画ファイル自体を外部サーバーへアップロードして圧縮する方式ではありません。
...
`premium-ui.js` を読み込んだあと、既存圧縮本体を保持した `compressor-core.js` を読み込みます。
...
14. v0.7以降は、既存機能を見た目のために削除しないこと。
...
- ブラウザ・動画コーデック・動画サイズによって圧縮速度は大きく変わります。
- 300MBを超える動画は自動圧縮対象外です。
- v0.7のプレミアムUIはコード・レスポンシブ条件まで確認済みですが、実ブラウザでの最終視覚確認は未実施です。
